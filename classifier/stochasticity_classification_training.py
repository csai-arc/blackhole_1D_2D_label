...
def objective(trial) :
    ...
    traindir = os.path.join(args.data, 'Synthetic_data_images')
    valdir = os.path.join(args.data, 'GRS1915_images')
    ...
    if args.pretrained:
        ...
        model = models.__dict__[args.arch](
                    ...
                    cardinality=args.cardinality,num_classes=2,
                )
    elif args.arch.startswith('effnetv2'):

        model = models.__dict__[args.arch](
                    num_classes=2, width_mult=1.,
                )
    ...
    if args.resume:
        ...
        logger.set_names(['Learning Rate', 'Train Loss', 'Valid Loss', 'Train Acc.', 'Valid Acc.', 'train_stoch_class_top1_avg', 'test_stoch_class_top1_avg'])
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        train_loss, train_acc, train_stoch_class_top1_avg = train(train_loader, model, criterion, optimizer, epoch, use_cuda)
        test_loss, test_acc, test_stoch_class_top1_avg = test(val_loader, model, criterion, epoch, use_cuda)
        
        # append logger file
        logger.append([lr, train_loss, test_loss, train_acc, test_acc, train_stoch_class_top1_avg, test_stoch_class_top1_avg])
...
def train(train_loader, model, criterion, optimizer, epoch, use_cuda):
    ...
    losses = AverageMeter()
    stoch_class_top1 = AverageMeter()
    stoch_class_top5 = AverageMeter()
    end = time.time()

    bar = Bar('Processing', max=len(train_loader))
    for batch_idx, (inputs, stoch_class_targets) in enumerate(train_loader):
        ...
        if use_cuda:
            ...
            stoch_class_targets = stoch_class_targets.cuda(non_blocking=True)
            
        inputs, stoch_class_targets = torch.autograd.Variable(inputs), torch.autograd.Variable(stoch_class_targets)

        # compute output
        stoch_class_outputs = model(inputs)   # to be changed by including outputs_hm1, outputs_hm2

        stoch_class_loss = criterion(stoch_class_outputs, stoch_class_targets)
        
        loss = (stoch_class_loss)
        


        # measure accuracy and record loss
        stoch_class_prec1, stoch_class_prec1_dum = accuracy(stoch_class_outputs.data, stoch_class_targets.data, topk=(1, 1))
        stoch_class_top1.update(stoch_class_prec1.item(), inputs.size(0))
        ...
        # plot progress
        bar.suffix  = '({batch}/{size})Data:{data:.3f}s|Batch:{bt:.3f}s|Total:{total:}|ETA:{eta:}|Loss:{losses:.4f}|top1:{stoch_class_top1: .4f}|'.format(
                    batch=batch_idx + 1,
                    ...
                    eta=bar.eta_td,
                    losses=losses.avg,
                    stoch_class_top1=stoch_class_top1.avg,
                    )
        bar.next()

    bar.finish()
    train_accuracy = (stoch_class_top1.avg)
    return (losses.avg, train_accuracy, stoch_class_top1.avg)
...
def test(val_loader, model, criterion, epoch, use_cuda):
    ...
    for batch_idx, (inputs, stoch_class_targets ) in enumerate(val_loader):
        # measure data loading time
        data_time.update(time.time() - end)

        if use_cuda:
            inputs, stoch_class_targets = inputs.cuda(non_blocking=True), stoch_class_targets.cuda(non_blocking=True)
        with torch.no_grad():
            inputs, stoch_class_targets = torch.autograd.Variable(inputs), torch.autograd.Variable(stoch_class_targets)

        # compute output
        stoch_class_outputs = model(inputs)
        print("pred: ",stoch_class_outputs.data)
        print("targ: ",stoch_class_targets.data)
        stoch_class_loss = criterion(stoch_class_outputs, stoch_class_targets)

        loss = (stoch_class_loss)# + yoga20_loss + yoga82_loss)
        ...
        bar.next()
    bar.finish()
    test_accuracy = (stoch_class_top1.avg)
    return (losses.avg, test_accuracy, stoch_class_top1.avg)
